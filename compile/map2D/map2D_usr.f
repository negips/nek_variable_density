...
!     local variables
      ...
      real radius(lx1*ly1*lz1,lelv)  ! to replace ym1
      integer nxyz
...
!        Radius in Y
         do el=1,nelv
         do il=1,nvert
           ycoord(il,el) = sqrt(yc(il,el)**2 + zc(il,el)**2)
         enddo
         enddo       
      endif
...
!     in general this coud involve some curvilinear transform
      ...
      elseif (idir.eq.3) then  ! uniform z
         ifc = 5
         nxyz = lx1*ly1*lz1
!        prabal      ! For curvilinear Coordinates
         do el=1,nelv   
         do il=1,nxyz
           radius(il,el) = sqrt(ym1(il,1,1,el)**2 + zm1(il,1,1,el)**2)
         enddo
         enddo   
         do el=1,nelv
            call ftovec(map_xm1(1,1,el),xm1,el,ifc,nx1,ny1,nz1)
!            call ftovec(map_ym1(1,1,el),ym1,el,ifc,nx1,ny1,nz1)
            call ftovec(map_ym1(1,1,el),radius,el,ifc,nx1,ny1,nz1)
         enddo
      endif
